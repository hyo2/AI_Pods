...
class PromptGenerationNode:
    ...
    def generate_prompt_from_plan(
        ...
        Args:
            plan: 이미지 계획
            visual_guidelines: Global Visual Guidelines (dict)
...
{{
  ...
  "lighting": "조명 상세 설명",
  "color_usage": "주요 색상",
  "image_prompt": "완전한 한글 프롬프트 (200-300자)"
}}
...
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"❌ 프롬프트 생성 최종 실패 ({plan.image_id})")
                    raise RuntimeError(f"프롬프트 생성 실패 ({plan.image_id}): JSON 파싱 에러 ({str(e)})")
            
            except KeyError as e:
                print(f"⚠️  필수 필드 누락 (시도 {attempt + 1}/{max_retries}, {plan.image_id}): {str(e)}")
                if attempt < max_retries - 1:
                    continue
                else:
                    print(f"❌ 프롬프트 생성 최종 실패 ({plan.image_id})")
                    raise RuntimeError(f"프롬프트 생성 실패 ({plan.image_id}): 필수 필드 누락 ({str(e)})")
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            state with image_prompts added
        """
